...
export default function initOptimisationsController(db) {
  ...
  const showWorker = async (request, response) => {
    const firstOfMonthDate = new Date(`${request.params.year}-${Number(request.params.month) + 1}-1`);
    ...
    try {
      ...
      const successMessage = `Successful retrieval of optimisations by Worker ID ${request.params.workerId} on ${firstOfMonthString} ${request.params.year}!`;
      response.send({
        message: successMessage,
        ...newObj,
        workers,
